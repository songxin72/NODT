...
class GRUCell(nn.Module):
    ...
    def forward(self, inputs, hidden):  # i:[128, 16] [128, 320]
        if self.gate_type == 'tensor':
            # reshape input
            # [B H]
            blk_input = inputs.unsqueeze(2).permute(1, 0, 2)  # [16, 128, 1]
            mv_input = blk_input.unsqueeze(2)  # [16, 128, 1, 1]
            # print(blk_input.shape, mv_input.shape)
            # reshape hidden
            B = hidden.size()[0]
            blk_h = torch.unsqueeze(hidden, dim=2)  # [128, 320, 1]
            blk_h2 = blk_h.view(B, -1, self._n_var)  # [128, 20, 16]
            mv_h = blk_h2.permute(2, 0, 1).unsqueeze(2)  # [16, 128, 1, 20]

            gate_tmp_I2H = (mv_input * self.gate_w_I2H).sum(3)  # o:[16, 128, 40]
            gate_tmp_H2H = (mv_h * self.gate_w_H2H).sum(3)  # o:[16, 128, 40]
            # [V 1 B 2D]
            gate_tmp_x = torch.chunk(gate_tmp_I2H, self._n_var, dim=0)  # 16个[1, 128, 640]
            gate_tmp_h = torch.chunk(gate_tmp_H2H, self._n_var, dim=0)

            # [1 B 2H]
            g_res_x = torch.cat(gate_tmp_x, dim=2)  # [1, 128, 640]
            g_res_h = torch.cat(gate_tmp_h, dim=2)  # [1, 128, 640]
            gate_res_x = g_res_x.squeeze(0)
            gate_res_h = g_res_h.squeeze(0)

            # [B 2H;
            res_gate = gate_res_x + gate_res_h + self.gate_bias  # [128, 640]
            z, r = torch.chunk(res_gate, 2, dim=1)  # [128, 320], [128, 320]
            blk_r = torch.unsqueeze(F.sigmoid(r), dim=2)  # gate模块结束 [128, 320, 1]
            blk_r2 = blk_r.view(B, -1, self._n_var)  # [128, 20, 16]
            mv_r = blk_r2.permute(2, 0, 1).unsqueeze(2)  # [16, 128, 1, 20]
            ...
            g = update_res_x + update_res_h + self.update_bias  # [128, 320]
            new_h = F.sigmoid(z) * hidden + (1 - F.sigmoid(z)) * F.tanh(g)  # 更新特征
        else:
...
class ODEfunc(nn.Module):
    ...
    def forward(self, t, h):
        ...
        u = torch.tanh(x + self.lin_hh(r * h))  # 对应论文公式3

        dh = (1 - z) * (u - h)  # 对应论文中的微分公式
        return dh
...
class attention2_mix(nn.Module):
    ...
    def forward(self, att_inputs):
        # att_h_tensor = torch.stack(att_inputs, dim=1)
        att_h_tensor = att_inputs  # [128, 20, 320]
        att_h_list = torch.split(att_h_tensor, [self.per_vari_dim] * self.input_dim, 2)  # 16个[128, 20, 20]
        h_att_temp_input = att_h_list
        # temporal attention
        # [V B T D] [head_dim, batch_size, time_step, per_dim] [16, 128, 20, 20] [26, 128, 20, 10]
        tmph = torch.stack(h_att_temp_input, dim=0)
        # [V B T-1 D], [V, B, 1, D]
        # tmph_before, tmph_last = torch.split(tmph, [self.steps - 1, 1], 2)
        # -- temporal logits
        if self.temp_type == 'temp_loc':  # 默认
            # [V, B, T-1] i:[head_dim, batch_size, time_step, per_dim] o:[16, 128, 20] [head_dim, batch_size, time_step]
            temp_logit = torch.tanh((tmph * self.att_w_temp).sum(3) + self.bias_temp)
        else:
            print('\n [ERROR] temporal attention type \n')
            temp_logit = 'nan'

        temp_weight = torch.softmax(temp_logit, dim=-1)  # 计算权重 o:[16, 128, 20]  对应论文公式5
        # temp_before [V B T-1 D], temp_weight [V B T-1] o:[16, 128, 20]
        tmph_cxt = (tmph * temp_weight.unsqueeze(-1)).sum(2)
        # tmph_last = tmph_last.squeeze(2) o:[128, 16, 20]
        v_temp = (tmph * temp_weight.unsqueeze(-1)).sum(3).permute(1, 0, 2)

        # [V B 2D]
        # h_temp = torch.cat((tmph_last, tmph_cxt), 2)
        h_temp = tmph_cxt  # [16, 128, 20]
        # variable attention
        if self.vari_type == 'vari_loc_all':  # 默认
            # o: [128, 16, 1] 对应论文公式6
            vari_logits = torch.tanh(((h_temp * self.aug_w_vari).sum(2, keepdim=True) + self.bias_vari).permute(1, 0, 2))
            # vari_logits = torch.tanh(((h_temp * self.aug_w_vari).sum(2, keepdim=True)).permute(1, 0, 2))
        ...
        c_t = (v_temp * vari_weight).sum(2)  # o:[128, 16] 对应论文公式7
        qz_out = self.fc(c_t)
